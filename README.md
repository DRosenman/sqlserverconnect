...
Here’s a short example demonstrating how to connect, query, and
disconnect:
...
`db_connect()` supports both DBI and pooled connections. Set
`pool = TRUE` to use a pool, or leave it as the default (`FALSE`) for a
regular DBI connection.
...
To use the DBI package for connections, keep `pool = FALSE` (the default
setting). To use the pool package for connections, set `pool = TRUE`.
